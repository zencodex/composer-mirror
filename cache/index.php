...
$file = ($ext === 'zip' ? rtrim($config->distdir, '/') : rtrim($config->cachedir, '/')) . "/" . ltrim($input, "/");

if (in_array($ext, ['zip', 'json'])) {
    try {
        if (file_exists($file)) {
            $distUrl = $file;
            $contents = file_get_contents($distUrl);
        } else {
        ...
        if (str_contains($distUrl, 'composer-merge-plugin')) {
            file_put_contents(__DIR__ . "/download.log", var_export([
                $file,
                $input,
                $distUrl,
                $e->getMessage(),
